...
function prepareMask(maskImg,width,height){
...
async function recolorCanvas(img,mode,colorA,colorB,colorAName='',colorBName=''){
  ...
  try{
    ...
    let maskA=null,maskB=null;

    if(mode==='plexy'){
      const mask=await loadImage('colors/plexy_colors/mask.png');
      maskA=prepareMask(mask,width,height);
    }else if(mode==='wood'){
      const mask=await loadImage('colors/3od_colors/mask.png');
      maskA=prepareMask(mask,width,height);
    }else if(mode==='combo-different'){
      const plexyMask=await loadImage('colors/3od_plexy_colors_different/plexy_mask.png');
      const woodMask=await loadImage('colors/3od_plexy_colors_different/wood_mask.png');
      maskA=prepareMask(plexyMask,width,height);
      maskB=prepareMask(woodMask,width,height);
    }else if(mode==='combo-uniform'){
      const mask=await loadImage('colors/3od_plexy_colors_uniform/mask.png');
      maskA=prepareMask(mask,width,height);
    }

    const aMirror=isMirrorColor(colorAName);
    const bMirror=isMirrorColor(colorBName);
    ...
    for(let i=0;i<px.length;i+=4){
      ...
      // Keep the gold decorative pieces/clasp untouched.
      ...
      if(mode==='plexy'){
        maskAlpha=maskA[p*4+3];
        if(maskAlpha>0 && !isGold){
          target=colorA;
          mirrorName=colorAName;
        }
      }else if(mode==='wood'){
        maskAlpha=maskA[p*4+3];
        if(maskAlpha>0 && !isGold){
        ...
        const plexyAlpha=maskA[p*4+3];
        const woodAlpha=maskB[p*4+3];

        if(plexyAlpha>0 && !isGold){
          target=colorA;
          mirrorName=colorAName;
          maskAlpha=plexyAlpha;
        }else if(woodAlpha>0 && !isGold){
          target=colorB;
          mirrorName=colorBName;
          maskAlpha=woodAlpha;
        }
      }else if(mode==='combo-uniform'){
        maskAlpha=maskA[p*4+3];
      ...
      if(isMirrorColor(mirrorName)){
        [rr,gg,bb]=mirrorRgb(mirrorName,Math.floor(p/width),height);
      }else{
        const rgb=hexRgb(colorAName===mirrorName?colorA:target);
        const tint=rgb||target;
        applyTint(px,i,tint,r,g,bl);
        continue;
      }

      // Respect the mask edges instead of painting outside the product.
    ...
    // If a mask cannot be loaded, keep the original product image visible.
...
async function updateColorPreview(){
  ...
  // Ready-made mirror images are used where they exist.
  if(mode!=='combo-different'){
    const mirrorKey=state.category==='plexi'
      ? `plexy|${colorAName}`
      : state.category==='wood'
      ? `wood|${colorAName}`
      : `combo-uniform|${colorAName}`;

    const mirror=mirrorAssets[mirrorKey];
    if(mirror && isMirrorColor(colorAName)){
      showPreviewImage(mirror);
      return;
    }
  }
