...
function showPreviewImage(src,showBadge=false){
  const canvas=$('#colorPreviewCanvas');
  const img=$('#modalImg');
  if(canvas){
    canvas.classList.remove('visible');
...
function showPreparedColorImage(src){
...
async function updateColorPreview(){
  ...
  // Plexy and wood color folders contain complete ready-made preview images.
  // Use the exact selected image instead of rebuilding it on the canvas.
  if(state.category==='plexi'){
    const color=state.colorSelections.plexi;
    if(!color){showPreviewImage(colorBaseAssets.plexy);return;}
    const src=colorAssetPath(layeredColorAssets.plexy.root,color);
    if(src){showPreparedColorImage(src);return;}
    showPreviewImage(colorBaseAssets.plexy);
    return;
  }
  ...
  if(state.category==='wood'){
    ...
    const src=colorAssetPath(layeredColorAssets.wood.root,color);
    if(src){showPreparedColorImage(src);return;}
    showPreviewImage(colorBaseAssets.wood);
    return;
  }

  // Combo products still use the transparent material layers.
