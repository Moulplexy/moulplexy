...
const colorBaseAssets={
  plexy:'colors/plexy_colors/base.png',
  wood:'colors/3od_colors/base.png',
  'combo-different':'colors/3od_plexy_colors_different/base.png',
  'combo-uniform':'colors/3od_plexy_colors_uniform/base.png'
};
...
function hexRgb(hex){const m=hex.replace('#','');return {r:parseInt(m.slice(0,2),16),g:parseInt(m.slice(2,4),16),b:parseInt(m.slice(4,6),16)}}
function isMirrorColor(color){return typeof color==='string' && color.toLocaleLowerCase().includes('miroir')}
function mirrorRgb(color,y,height){
  const key=String(color||'').toLocaleLowerCase();
  const palettes={
    'الغوز miroir':[[225,170,190],[130,65,90],[235,190,205]],
    'argenté miroir':[[230,235,240],[120,125,135],[250,250,250]],
    'doré miroir':[[240,205,95],[150,100,20],[255,230,135]],
    'bronze miroir':[[195,125,75],[95,50,28],[225,155,105]]
  };
  const pal=palettes[key]||palettes['argenté miroir'];
  const t=Math.max(0,Math.min(1,y/Math.max(1,height-1)));
  const u=t<.5?t*2:(t-.5)*2;
  const a=t<.5?pal[0]:pal[1], b=t<.5?pal[1]:pal[2];
  const band=.5+.5*Math.cos(t*5.2*Math.PI), hi=1+.25*band;
  return [a[0]*(1-u)+b[0]*u,a[1]*(1-u)+b[1]*u,a[2]*(1-u)+b[2]*u].map(v=>Math.min(255,v*hi));
}
function recolorCanvas(img, mode, colorA, colorB, colorAName='', colorBName=''){
  const canvas=$('#colorPreviewCanvas'); if(!canvas)return;
  const ctx=canvas.getContext('2d',{willReadFrequently:true});
  canvas.width=img.naturalWidth; canvas.height=img.naturalHeight; ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0);
  const data=ctx.getImageData(0,0,canvas.width,canvas.height), px=data.data;
  const a=colorA?hexRgb(colorA):null, b=colorB?hexRgb(colorB):null;
  const aMirror=isMirrorColor(colorAName), bMirror=isMirrorColor(colorBName);
  for(let i=0;i<px.length;i+=4){
    ...
    const inTop=(x>150 && x<750 && y>175 && y<432);
    const inBody=(x>180 && x<720 && y>430 && y<592);
    let target=null, mirrorName='';
    if(mode==='plexy' && inTop && !isGold){target=a; mirrorName=colorAName}
    else if(mode==='wood' && inBody && !isGold){target=a; mirrorName=colorAName}
    else if(mode==='combo-different'){
      if(inTop && !isGold){target=a; mirrorName=colorAName}
      else if(inBody && !isGold){target=b; mirrorName=colorBName}
    }else if(mode==='combo-uniform' && (inTop||inBody) && !isGold){target=a; mirrorName=colorAName}
    if(target){
      let rr,gg,bb;
      if(isMirrorColor(mirrorName)){
        [rr,gg,bb]=mirrorRgb(mirrorName,y,canvas.height);
      }else{
        const lum=(0.2126*r+0.7152*g+0.0722*bl)/255;
        const shade=Math.max(.38,Math.min(1.08,lum*1.15));
        rr=target.r*shade; gg=target.g*shade; bb=target.b*shade;
      }
      px[i]=Math.min(255,rr); px[i+1]=Math.min(255,gg); px[i+2]=Math.min(255,bb);
    }
  }
  ctx.putImageData(data,0,0);
  canvas.classList.add('visible'); $('#modalImg').style.visibility='hidden'; $('#previewBadge').hidden=false;
}
function showPreviewImage(src){
  const canvas=$('#colorPreviewCanvas'); if(canvas){canvas.classList.remove('visible');canvas.width=1;canvas.height=1}
  $('#modalImg').style.visibility='visible'; $('#previewBadge').hidden=true;
  const img=new Image(); img.onload=()=>{if(canvas){canvas.width=img.naturalWidth;canvas.height=img.naturalHeight;const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(img,0,0);canvas.classList.add('visible');$('#modalImg').style.visibility='hidden';$('#previewBadge').hidden=false;}}; img.src=encodeURI(src);
}
function updateColorPreview(){
  if(!state.selected)return;
  let mode, colorA, colorB, colorAName='', colorBName='';
  if(state.category==='plexi'){mode='plexy';colorAName=state.colorSelections.plexi;colorA=standardColorHex[colorAName]||'#ddd'}
  else if(state.category==='wood'){mode='wood';colorAName=state.colorSelections.wood;colorA=standardColorHex[colorAName]||'#8a5a3b'}
  else if(state.comboMode==='uniform'){mode='combo-uniform';colorAName=state.colorSelections.comboUniform;colorA=standardColorHex[colorAName]||'#b27a48'}
  else {mode='combo-different';colorAName=state.colorSelections.comboPlexi;colorBName=state.colorSelections.comboWood;colorA=standardColorHex[colorAName]||'#ddd';colorB=standardColorHex[colorBName]||'#8a5a3b'}
  const baseSrc=colorBaseAssets[mode];
  if(!colorAName || (mode==='combo-different'&&!colorBName)){
    showPreviewImage(baseSrc);
    return;
  }
  // Dedicated ready-made mirror image for single-material and uniform selections.
  if(mode!=='combo-different'){
    const mirrorKey=state.category==='plexi'?`plexy|${colorAName}`:state.category==='wood'?`wood|${colorAName}`:`combo-uniform|${colorAName}`;
    const mirror=mirrorAssets[mirrorKey];
    if(mirror && isMirrorColor(colorAName)) return showPreviewImage(mirror);
  }
  const base=new Image();
  base.onload=()=>recolorCanvas(base,mode,colorA,colorB,colorAName,colorBName);
  base.src=encodeURI(baseSrc);
}
