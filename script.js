...
function renderGallery(){
  ...
  items.forEach((item,i)=>{
    ...
    b.type="button";
    b.innerHTML=`<img loading="lazy" src="${encodeURI(item.url)}" alt="${escapeHtml(categoryMeta[state.category][state.lang==="ar"?"ar":"fr"])} ${i+1}">`;
...
const colorBaseAssets={
  plexy:'colors/plexy/photo plexy ra2isiya.png',
...
function selectedColorPhoto(){
  if(state.category==='plexy') return colorPhotoAssets.plexy[state.colorSelections.plexi] || colorBaseAssets.plexy;
...
function renderMaterialPicker(container, materialKey, title, groups){
  ...
  container.innerHTML=`
    <div class="picker-head"><h4>${escapeHtml(title)}</h4><span class="selected-color">${selected?escapeHtml(selected):colorLabels.choose}</span></div>
      ...
      <div class="color-options">
        ${group.items.map(color=>`
          <button type="button" class="color-option ${selected===color?'active':''}" data-color="${escapeHtml(color)}" aria-selected="${selected===color}">
            <span class="color-dot" aria-hidden="true"></span><span>${escapeHtml(color)}</span>
          </button>`).join('')}
      </div>
    </div>`).join('');
...
function colorSummary(){
  if(state.category==='plexy')return state.colorSelections.plexi||'غير محدد';
  if(state.category==='wood')return state.colorSelections.wood||'غير محدد';
  return `العود: ${state.colorSelections.comboWood||'غير محدد'} | البليكسي: ${state.colorSelections.comboPlexi||'غير محدد'}`;
}
...
function buildWhatsAppMessage(){
  ...
  const colorUrl=state.selected ? new URL(selectedColorPhoto(),location.href).href : "";
  const catalogUrl=state.selected ? new URL(state.selected.url,location.href).href : "";

  if(state.lang==="ar"){
    return `السلام عليكم، أريد طلب نموذج من MOULE PLEXY.
الفئة: ${cat}
اختيار اللون: ${colorSummary()}
${colorUrl?`📌 Photo from colores:
${colorUrl}`:""}
${catalogUrl?`📌 Photo from catalog:
...
أعلم أن الأداء قبل التنفيذ، ولن يتم البدء في إنجاز الطلب إلا بعد التوصل بالأداء. يمكن أداء 50% قبل الإنجاز، والباقي قبل التسليم.
...
Je reconnais que le paiement est requis avant l’exécution de la commande. Nous pouvons convenir de 50 % avant la réalisation et du solde avant la livraison.
