...
function renderMaterialChoices(){
  ...
  if(state.category==='plexi'){
    mode.hidden=true; separate.hidden=true; uniform.hidden=true; single.hidden=false;
    ...
    if(state.comboMode==='separate'){
      separate.hidden=false; uniform.hidden=true;
      ...
      separate.hidden=true; uniform.hidden=false;
...
$$('input[name="comboMode"]').forEach(r=>r.addEventListener('change',()=>{state.comboMode=r.value;renderMaterialChoices()}));
